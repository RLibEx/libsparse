...
static int _ftruncate(int fd, int64_t len)
{
    HANDLE h = (HANDLE)_get_osfhandle(fd);
    LARGE_INTEGER len64;
    len64.QuadPart = len;
    SetFilePointerEx(h, len64, NULL, FILE_BEGIN);
    SetEndOfFile(h);
    return len;
}

#define lseek64 _lseeki64
#define ftruncate64 _ftruncate
#define mmap64 mmap
...
static int callback_file_skip(struct output_file *out, int64_t off)
{
	...
	while (off > 0) {
		to_write = __min(off, (int64_t)INT_MAX);
		...
		uint32_t fill_val)
{
	...
	while (len) {
		write_len = __min(len, out->block_size);
